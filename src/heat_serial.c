/*
 * Serial implementation of 2D Heat Equation solver
...
double compute_step(double **grid, double **grid_new, int n, double factor);
...
void log_performance_serial(int n, int steps, double elapsed, double memory_mb);
...
int main(int argc, char *argv[]) {
    ...
    printf("=== Serial Heat Equation Solver ===\n");
    ...
    for (int step = 0; step < steps; step++) {
        max_diff = compute_step(grid, grid_new, n, factor);
    ...
    printf("Approx memory used (2 grids): %.2f MB\n", memory_mb);
    ...
    // Save results
    save_results(grid, n, "results/serial_result.txt");

    log_performance_serial(n, steps, elapsed, memory_mb);
...
void log_performance_serial(int n, int steps, double elapsed, double memory_mb) {
    FILE *fp = fopen("results/serial_perf.log", "a");
    if (fp == NULL) {
        printf("Warning: Could not open serial_perf.log for writing\n");
        return;
    }

    fprintf(fp, "n=%d steps=%d time=%.6f time_per_step=%.8f memMB=%.2f\n",
            n, steps, elapsed, elapsed / steps, memory_mb);
