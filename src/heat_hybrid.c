/*
 * Hybrid MPI+OpenMP parallel implementation of 2D Heat Equation solver
 * Uses finite difference method with explicit time stepping
 * Parallelization: 
 *   - MPI: Domain decomposition (1D decomposition in rows) between nodes
 *   - OpenMP: Data parallelism within each MPI rank's domain
 */
...
double compute_step_hybrid(double **grid, double **grid_new, int local_rows, int n, 
...
void log_performance_hybrid(int n, int steps, int size, int num_threads,
                            double elapsed, double total_mem_mb, double per_rank_mem_mb);
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        printf("=== Hybrid MPI+OpenMP Heat Equation Solver ===\n");
        printf("Grid size: %d x %d\n", n, n);
        printf("Time steps: %d\n", steps);
        printf("Alpha: %.2f\n", ALPHA);
        printf("MPI processes: %d\n", size);
        printf("OpenMP threads per process: %d\n", num_threads);
        printf("Total parallel tasks: %d\n", size * num_threads);
        printf("Rows per process: ~%d\n", n / size);
        printf("MPI thread support level: %d (requested: %d)\n", provided, MPI_THREAD_FUNNELED);
    }
    ...
    for (int step = 0; step < steps; step++) {
        ...
        // Compute new values using hybrid parallelization
        max_diff = compute_step_hybrid(grid, grid_new, local_rows, n, factor, rank, size);
    ...
    if (rank == 0) {
        ...
        double per_rank_mem_mb = total_mem_mb / size;
        printf("Approx total memory over all ranks (2 global grids): %.2f MB\n", total_mem_mb);
        printf("Approx memory per MPI rank (shared by %d threads): %.2f MB\n",
               num_threads, per_rank_mem_mb);
    }
    ...
    // Save results
    char filename[100];
    sprintf(filename, "results/hybrid_result_%dx%d.txt", size, num_threads);
    ...
    if (rank == 0) {
        ...
        double total_mem_mb = total_mem_bytes / (1024.0 * 1024.0);
        double per_rank_mem_mb = total_mem_mb / size;
        log_performance_hybrid(n, steps, size, num_threads,
                               elapsed, total_mem_mb, per_rank_mem_mb);
    }
...
void initialize_grid(double **grid, int local_rows, int n, int rank, int size) {
    ...
    int total_rows = local_rows + ghost_offset + (rank < size - 1 ? 1 : 0);
    
    // Initialize local domain to zero - parallelized with OpenMP
...
double compute_step_hybrid(double **grid, double **grid_new, int local_rows, int n, 
    ...
    // Parallelize with OpenMP, use reduction for max_diff
    ...
    // Reduce across MPI ranks to get global max_diff
...
void save_results(double **grid, int local_rows, int n, int rank, const char *filename) {
    // Only rank 0 writes the file header
    if (rank == 0) {
...
double verify_solution(double **grid, int local_rows, int n, int rank, int size) {
    ...
    // Compute local contributions with OpenMP
    int start_row = (rank == 0) ? 1 : ghost_offset;
    int end_row = (rank == size - 1) ? ghost_offset + local_rows - 1 : ghost_offset + local_rows;
    
    #pragma omp parallel for collapse(2) reduction(+:local_heat) reduction(max:local_symmetry_error)
    for (int i = start_row; i < end_row; i++) {
        for (int j = 1; j < n - 1; j++) {
            local_heat += grid[i][j];
        }
    ...
    // Reduce across MPI ranks to get global values
...
void log_performance_hybrid(int n, int steps, int size, int num_threads,
                            double elapsed, double total_mem_mb, double per_rank_mem_mb) {
    FILE *fp = fopen("results/hybrid_perf.log", "a");
    if (fp == NULL) {
        printf("Warning: Could not open hybrid_perf.log for writing\n");
        return;
    }

    fprintf(fp,
            "n=%d steps=%d ranks=%d threads=%d time=%.6f time_per_step=%.8f totalMemMB=%.2f memPerRankMB=%.2f\n",
            n, steps, size, num_threads, elapsed, elapsed / steps,
            total_mem_mb, per_rank_mem_mb);
